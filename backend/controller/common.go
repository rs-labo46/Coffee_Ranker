...
func mapError(err error) (int, string, string) {
	...
	case errors.Is(err, entity.ErrInvalidInput),
		...
		errors.Is(err, entity.ErrInvalidContentType),
		errors.Is(err, entity.ErrInvalidEventType),
		errors.Is(err, entity.ErrInvalidRatingScore),
		errors.Is(err, entity.ErrInvalidSearchCondition):
	...
	case errors.Is(err, entity.ErrUnauthorized),
		...
		errors.Is(err, entity.ErrRefreshTokenReuseDetected):
		return http.StatusUnauthorized, "unauthorized", "認証が必要です"

	case errors.Is(err, entity.ErrForbidden),
		errors.Is(err, entity.ErrLoginRequired),
	...
	case errors.Is(err, entity.ErrBeanNotFound),
		errors.Is(err, entity.ErrArticleNotFound),
		errors.Is(err, entity.ErrRankTargetNotFound),
		errors.Is(err, entity.ErrGuestSessionNotFound),
		errors.Is(err, entity.ErrSavedItemNotFound),
		errors.Is(err, entity.ErrRatingNotFound),
		errors.Is(err, entity.ErrModalCandidateNotFound),
		errors.Is(err, entity.ErrModalDisplayLogNotFound),
	...
	case errors.Is(err, entity.ErrEmailAlreadyExists),
		errors.Is(err, entity.ErrSavedItemAlreadyExists),
	...
	case errors.Is(err, entity.ErrBatchAlreadyRunning),
		errors.Is(err, entity.ErrBatchLockFailed):
...
func actorFromContext(c echo.Context) (usecase.Actor, error) {
	actor := usecase.Actor{
		...
		GuestSessionID: optionalGuestSessionID(c),
	}
...
func adminMeta(c echo.Context) (usecase.AdminMeta, error) {
	...
	return usecase.AdminMeta{
		AdminUserID: userID,
		AuditMeta:   auditMeta(c),
	}, nil
}
...
func auditMeta(c echo.Context) usecase.AuditMeta {
	return usecase.AuditMeta{
		RequestID:     optionalStringContext(c, ContextRequestIDKey),
		IPAddressHash: optionalStringContext(c, ContextIPHashKey),
	}
}
...
func setRefreshCookie(c echo.Context, cfg CookieConfig, token string) {
	...
	cookie := &http.Cookie{
		...
		HttpOnly: true,
		Secure:   cfg.Secure,
		SameSite: cookieSameSite(cfg, http.SameSiteStrictMode),
	}
	c.SetCookie(cookie)
}
...
func setGuestSessionCookie(c echo.Context, cfg CookieConfig, sessionKey string) {
	...
	cookie := &http.Cookie{
		...
		HttpOnly: true,
		Secure:   cfg.Secure,
		SameSite: cookieSameSite(cfg, http.SameSiteLaxMode),
	}
	c.SetCookie(cookie)
